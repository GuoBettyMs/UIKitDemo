...
//            print("scrollViewDidScroll, contentOffset.y: ", self.collectionV.contentOffset.y,", contentOffset: ", contentOffset)
            ...
            self.privatebtn.isHidden = self.collectionV.contentOffset.y >= contentOffset-5 ? false : true//为了privatebtn在collectionV滚动到差不多到底部时就显示,将contentOffset降低点 (contentOffset-5)
        }
    }
...
/// - Returns:
///  设置分区背景
extension ViewController: SectionDecorationLayoutDelegate {
...
extension ViewController: UITextFieldDelegate{
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        print("textFieldDidEndEditing text,", textfiled.text ?? "")
    }
