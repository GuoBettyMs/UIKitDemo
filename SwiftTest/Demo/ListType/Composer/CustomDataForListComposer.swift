...
class CustomDataForListComposer {
    ...
    static private func baseData() -> ListTypeBasicModel {
        ListTypeBasicModel()
            .sectionTitleArr([
                "Demo test",
            ...
            .cellTitleArr([
                [
                    ...
                    "世界时区选择器,自定义提示框",
                    "(滚轮选择项文本颜色与小数点文本颜色不一致)自定义滚轮+菜单跳转",
                    "多行文本分段控制器"
                ],
                [
                    "Column Chart---柱状图",
    ...
    static func table() -> ListTypeBasicModel {
        baseData().listType(.table)
    }
    
    static func collection() -> ListTypeBasicModel {
        baseData().listType(.collection)
    }
    
    static func none() -> ListTypeBasicModel {
